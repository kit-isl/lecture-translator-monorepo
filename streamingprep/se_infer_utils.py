...
def main_increase(args):
    chunk_size=1 # 1 second
    look_ahead_size=0.2 # 0.2 second
    model, df_state, suffix = init_df(
        args.model_base_dir,
        post_filter=args.pf,
        log_level=args.log_level,
    ...
    for i, file in enumerate(args.noisy_audio_files):
        ...
        audio, meta = load_audio(file, df_sr)
        list_audio = []
        for start_idx in range(0, audio.size(-1), chunk_size*df_sr):
            size = int(min(start_idx + (chunk_size + look_ahead_size) * df_sr, audio.size(-1)))
            print(start_idx, size)
            list_audio.append(audio[:, start_idx: size])
            if size == audio.size(-1):
                break
        # audio_chunks = 
        print(audio.shape)
        for idx, audio in enumerate(list_audio):
            ...
            audio = resample(audio, df_sr, meta.sample_rate)
            save_audio(
                file, audio, sr=meta.sample_rate, output_dir=args.output_dir, suffix=suffix+"_"+str(idx), log=False
            )
