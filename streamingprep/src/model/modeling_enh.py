...
class VoiceFilter(PreTrainedModel):
    ...
    def do_enh_batch(self, speechs, speaker_embeddings):
        assert speechs.size(-1) % self.wav_chunk_size == 0
        enh_speech = self.forward(
            speech=speechs,
            speech_lengths=torch.tensor([speechs.size(-1)] * len(speechs)),
            target_spk_embedding=speaker_embeddings
        ).output_speech[:, :speechs.size(-1)]
        return enh_speech
